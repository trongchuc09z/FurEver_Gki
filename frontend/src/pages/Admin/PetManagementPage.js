...
const ToggleStatusIcon = ({ isAvailable }) => (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M18.36 6.64a9 9 0 1 1-12.72 0" stroke={isAvailable ? "#ff5b35" : "#4CAF50"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
        <path d="M12 2v10" stroke={isAvailable ? "#ff5b35" : "#4CAF50"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
);
...
const PetManagementPage = () => {
    ...
    const [allPets, setAllPets] = useState([]);
    const [displayPets, setDisplayPets] = useState([]);
    const [filters, setFilters] = useState({ species: [], gender: [], adoptionFee: [], age: [], vaccinated: [], sterilized: [], adoptionStatus: [] });
...
useEffect(() => {
    ...
    if (searchTerm) {
        ...
        results = results.filter(pet =>
            (pet.name && pet.name.toLowerCase().includes(lowercasedTerm)) ||
            (pet.breed && pet.breed.toLowerCase().includes(lowercasedTerm))||
           (pet._id && pet._id.toString().includes(lowercasedTerm))
        );
    }

    // Lọc theo checkbox (với kiểm tra tồn tại)
    if (filters.species && filters.species.length > 0) {
        results = results.filter(pet => pet.species && filters.species.includes(pet.species));
    }
    
    if (filters.gender && filters.gender.length > 0) {
        results = results.filter(pet => pet.gender && filters.gender.includes(pet.gender));
    }
    
    if (filters.vaccinated && filters.vaccinated.length > 0) {
        results = results.filter(pet => filters.vaccinated.includes(String(pet.vaccinated)));
    }
    
    if (filters.sterilized && filters.sterilized.length > 0) {
        results = results.filter(pet => filters.sterilized.includes(String(pet.sterilized)));
    }

    if (filters.adoptionFee && filters.adoptionFee.length > 0) {
        results = results.filter(pet => {
            const fee = pet.adoptionFee || 0;
    ...
    if (filters.adoptionStatus && filters.adoptionStatus.length > 0) {
        results = results.filter(pet => pet.status && filters.adoptionStatus.includes(pet.status));
    }
    ...
    const handleToggleStatus = async (pet) => {
        ...
        const alertMessage = newStatus === 'adopted'
            ? 'Bạn có chắc muốn đánh dấu thú cưng này là "Đã có chủ" không?'
            : 'Bạn có chắc muốn đánh dấu thú cưng này là "Sẵn sàng" không?';
        if (window.confirm(alertMessage)) {
            const config = {
                headers: {
                    'Content-Type': 'application/json', // Gửi JSON, không phải FormData
                    Authorization: `Bearer ${userInfo.token}`,
                },
            };

            try {
                // Sử dụng PATCH để cập nhật một phần (chỉ status)
                ...
                setAllPets(allPets.map((p) => (p._id === data._id ? data : p)));
            } catch (error) {
                console.error('Lỗi khi cập nhật trạng thái:', error.response ? error.response.data : error.message);
                alert('Đã xảy ra lỗi. Vui lòng thử lại.');
            }
        }
    };
            ...
            <div className="page-header"> {/* THAY ĐỔI CLASS */}
                <div className="page-header-group"> {/* THAY ĐỔI CLASS */}
                    <div className="search-bar"> {/* THAY ĐỔI CLASS */}
                        <input 
                            type="text" 
                            placeholder="Tìm kiếm theo tên, giống loài hoặc ID" 
                            value={searchInput} 
            ...
            {showFilter && (
                <div className="filter-form-container">
                    <div className="filter-grid">
                        ...
                        <label className="group-label">Trạng thái nhận nuôi</label>
                        <div className="options">
                            {createCheckbox('adoptionStatus', 'available', 'Sẵn sàng')}
                            {createCheckbox('adoptionStatus', 'adopted', 'Đã có chủ')}
                        </div>
                    </div>
                    ...
                    <div className="pm-pet-card" key={pet._id}>
                        ...
                        <div className="pm-pet-content">
                            ...
                            <div className="pm-pet-details">
                                ...
                                <p><strong>Trạng thái: </strong> {pet.status === 'available' ? 'Sẵn sàng' : 'Đã có chủ'}                    </p>
                            </div>
                            <div className="pm-pet-actions">
                                <button 
                                    onClick={() => handleToggleStatus(pet)} 
                                    title={pet.status === 'available' ? 'Đánh dấu là Đã có chủ' : 'Đánh dấu là Sẵn sàng'}
                                    className={pet.status === 'available' ? 'adopted' : 'available'}
                                >
                                    <ToggleStatusIcon isAvailable={pet.status === 'available'} />
                                </button>
